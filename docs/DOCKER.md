...
需要暴露给宿主/局域网的只有博客自己的入口端口，其余全部走 Docker 内部网络：

| 端口 | 用途 | 何时需要 |
|------|------|---------|
| `WEB_PORT`（默认 8080） | 博客 HTTP 入口 → 容器内 `app:3000` | 始终需要 |
| `HTTPS_PORT`（默认 8443） | 内置 Caddy 的 HTTPS 入口 | 用 `--profile https` 时 |
| `MINIO_API_PORT` / `MINIO_CONSOLE_PORT`（默认 9000 / 9001） | MinIO（`--profile minio`）。附件改为 S3 存储后，**浏览器要能直接访问 API 端口**才能显示图片 | 用 `--profile minio` 时 |
| `DB_PORT` | MySQL，**compose 默认不映射** | 一般不需要 |
| 3000 / 3306 / 33060 / 容器内 8443 | 仅供内部网络 | 不需要 |

- **飞牛自身占用**：`443` 与 `80`（系统 nginx，访问会 302 跳到管理界面）、`5667`（管理界面）。
  `WEB_PORT` / `HTTPS_PORT` 避开这些即可。
- 其它常见冲突（同机应用/容器）：9000（原生 MinIO）、5432（PostgreSQL）、8033、6680、2049（NFS）、
  139/111（SMB）。改 `.env` 里对应端口一条即可，容器内部端口不变。
- 从外网访问时只放通 HTTPS 入口（8443，或反向代理的 443），**MySQL 一律不要开公网**（PRD 5.5）。
- 应用自身的出站需求：构建/拉镜像时访问镜像加速与包源；启用通知后访问 SMTP（465/587）与飞书（443）。
- 需要用客户端连库时，把 `docker-compose.yml` 里 `db` 的 `ports` 注释取消，并确认只在内网开放。
...
| app 日志 `启动引导失败：Failed query: ...` 并跟出 MySQL 错误（如 `Duplicate check constraint name`、`Table ... already exists`） | 迁移语句在 MySQL 上执行失败。先在仓库里跑 `node scripts/audit-migrations.mjs` 检查迁移链能否从零重放（迁移文件被手工改过时会重复建约束/表，只有全新库才会暴露）。修好之后**必须先清空这个半应用的库**再启动，否则会一直撞「表已存在」：`docker compose down && sudo rm -rf <DATA_DIR>/mysql && docker compose up -d`（仅在尚未写入正式数据时可这么做） |
...
| `https://<地址>:8443` 打不开：`curl` 报 `tlsv1 alert internal error`，浏览器报连接失败（明文访问反而返回 400） | Caddy 没拿到证书——站点地址缺少主机名。检查 `.env` 的 `HTTPS_HOST` 是否等于浏览器访问用的地址（IP 或域名），改完 `docker compose --profile https up -d` 重建容器；`docker compose --profile https logs https --tail 30` 会打印 Caddy 的实际报错 |
| app 显示 `unhealthy` 但站点能正常打开 | 健康检查窗口（宽限 180s + 5 次重试）在慢速 NAS 的"首次启动 + 首次迁移建表"下可能耗尽，服务就绪后下一次探针会自动翻回 `healthy`。持续 unhealthy 才需要看 `docker compose logs app` 与 `curl -i http://127.0.0.1:<WEB_PORT>/console/login` |
